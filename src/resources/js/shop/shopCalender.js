  $(document).ready(function () {
    ...
    $.ajax({
        ...
        cache: false,
        dataType:'json',
        data:{ 'today' : today , 'shop_id' : shop_id },
    })
    .done(function(response) { 
        //通信成功時の処理
        if(response) {
            // 成功の処理
            $('#day #shop_amount').html(response.amount);
            $('#day #shop_dayBeforeRatio .yen').html('¥' + response.dayBeforeRatio);
            $('#day #shop_quantity').html(response.quantity);
            $('#day #shop_sellers_reward').html('¥' + response.sellers_reward);
        } else {
            // エラーの処理
        }
    })
    ...
    $('.hasDatepicker').change(function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType:'json',
            data:{ 'selectDay' : selectDay , 'shop_id' : shop_id },
        })
        .done(function(response) { 
            //通信成功時の処理
            if(response) {
    ...
    $('.prev_day').on('click', function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType:'json',
            data:{ 'beforeDay' : beforeDay , 'shop_id' : shop_id },
        })
        .done(function(response) { 
            //通信成功時の処理
            if(response) {
    ...
    $('.next_day').on('click', function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType:'json',
            data:{ 'afterDay' : afterDay , 'shop_id' : shop_id },
        })
        .done(function(response) { 
            //通信成功時の処理
            if(response) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType:'json',
        data:{ 'month' : month , 'shop_id' : shop_id },
    })
    .done(function(response) { 
        //通信成功時の処理
        if(response) {
            // 成功の処理
            $('#month #shop_amount').html(response.amount);
            $('#month #shop_dayBeforeRatio .yen').html('¥' + response.dayBeforeRatio);
            $('#month #shop_quantity').html(response.quantity);
            $('#month #shop_sellers_reward').html('¥' + response.sellers_reward);
        } else {
            // エラーの処理
        }
    })
    ...
    $('.hasMonthpicker').change(function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType:'json',
            data:{ 'selectMonth' : selectMonth , 'shop_id' : shop_id },
        })
        .done(function(response) { 
            //通信成功時の処理
            if(response) {
    ...
    $('.prev_month').on('click', function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType:'json',
            data:{ 'beforeMonth' : beforeMonth , 'shop_id' : shop_id },
        })
        .done(function(response) { 
            //通信成功時の処理
            if(response) {
    ...
    $('.next_month').on('click', function () {
        ...
        $.ajax({
            ...
            cache: false,
            dataType:'json',
            data:{ 'afterMonth' : afterMonth , 'shop_id' : shop_id },
        })
        .done(function(response) { 
            //通信成功時の処理
            if(response) {
